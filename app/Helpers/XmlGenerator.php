...
class XmlGenerator
{
    ...
    public static function generateInvoiceXml($venta, array $detalles, $empresa, $sucursal, $cuotas): DOMDocument
    {
        ...
        if ($venta->idFormaPago == FormaPago::CREDITO_FIJO) {
            ...
            foreach ($cuotas as $value) {
                ...
                $cbc->setAttribute('currencyID', $venta->codiso);
                $cbc = $PaymentTerms->appendChild($cbc);
                $cbc = $xml->createElement('cbc:PaymentDueDate', date("Y-m-d", strtotime($value->fecha)));
                $cbc = $PaymentTerms->appendChild($cbc);
            }
        }
        ...
        $cbc = $xml->createElement('cbc:LineExtensionAmount', number_format(round($subTotal, 2, PHP_ROUND_HALF_UP), 2, '.', '')); //
    ...
    public static function generateSummaryDocumentsXml($venta, $detalles, $empresa, $correlativo, $currentDate)
    {
        $sub_total = array_reduce($detalles, function ($acumulador, $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $sub_total = floatval($producto->cantidad) * (floatval($producto->precio) / ($igv  + 1));
