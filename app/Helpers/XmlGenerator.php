...
use App\Constants\SunatModalidadTraslado;
use App\Constants\SunatMotivoTraslado;
...
class XmlGenerator
{
    ...
    public static function createInvoiceXml(Venta $venta, array $detalles, array $cuotas, Empresa $empresa, Sucursal $sucursal): DOMDocument
    {
        ...
        if ($venta->idFormaPago == FormaPago::CONTADO || $venta->idFormaPago == FormaPago::ADELANTADO) {
     ...
     * @param Detalle[] $detalles
     */
    public static function createSummaryDocumentsXml(Venta $venta, array $detalles, Empresa $empresa, int $correlativo, DateTime $currentDate)
    {
        $sub_total = array_reduce($detalles, function ($acumulador, Detalle $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $sub_total = floatval($producto->cantidad) * (floatval($producto->precio) / ($igv  + 1));
            return $acumulador + $sub_total;
        });
     ...
     */
    public static function createDespatchAdviceXml(GuiaRemision $guiaRemision, array $detalles, Empresa $empresa)
    {
        ...
        if (SunatMotivoTraslado::VENTA == $guiaRemision->codigoMotivoTraslado) {
        ...
        if ($guiaRemision->codigoModalidadTraslado == SunatModalidadTraslado::TRANSPORTE_PUBLICO) {
            /**
             * INDICADORES
             */
            $cac_shipment->appendChild(
                $xml->createComment('Indicador de retorno de vehiculo con envases o embalajes vacios')
            );
            ...
            $cac_shipment->appendChild(
                $xml->createComment('Indicador de retorno de vehiculo vacio')
            );
            ...
            // Solo para motivo 04
            if ($guiaRemision->codigoMotivoTraslado === SunatMotivoTraslado::ENTRE_ESTABLECIMIENTOS) {
                $cbc = $xml->createElement(
                    'cbc:AddressTypeCode',
                    $guiaRemision->codigoAnexoLlegada
                );
                $cbc->setAttribute('listID', $empresa->documento);
                $cbc->setAttribute('listAgencyName', 'PE:SUNAT');
                $cbc->setAttribute('listName', 'Establecimientos anexos');
                $cac_delivery_address->appendChild($cbc);
            }
            ...
            if ($guiaRemision->codigoMotivoTraslado === SunatMotivoTraslado::ENTRE_ESTABLECIMIENTOS) {
                $cbc = $xml->createElement(
                    'cbc:AddressTypeCode',
                    $guiaRemision->codigoAnexoPartida
                );
                $cbc->setAttribute('listID', $empresa->documento);
                $cbc->setAttribute('listAgencyName', 'PE:SUNAT');
                $cbc->setAttribute('listName', 'Establecimientos anexos');
                $cac_despatch_address->appendChild($cbc);
            }
            ...
            $cac_shipment->appendChild($cac_transport_handling_unit);
        }

        if ($guiaRemision->codigoModalidadTraslado == SunatModalidadTraslado::TRANSPORTE_PRIVADO) {
            /**
             * INDICADORES
             */
            ...
            $cbc->appendChild($xml->createCDATASection($guiaRemision->numeroPlaca));
            $cac_transport_equipment->appendChild($cbc);

            // $cac_shipment->appendChild($xml->createComment('AUTORIZACIONES ESPECIALES - VEHICULO PRINCIPAL'));
            $cac_shipment_document_reference = $xml->createElement('cac:ShipmentDocumentReference');
            $cbc = $xml->createElement('cbc:ID');
            $cbc->setAttribute('schemeID', '06');
            $cbc->setAttribute('schemeName', 'Entidad Autorizadora');
            $cbc->setAttribute('schemeAgencyName', 'PE:SUNAT');
            $cac_shipment_document_reference->appendChild($cbc);

            $cac_transport_equipment->appendChild($cac_shipment_document_reference);

            $cac_transport_handling_unit->appendChild($cac_transport_equipment);
        }
    ...
    public static function generateCreditoNoteXml($notaDeCredito, array $detalles, $empresa, $sucursal): DOMDocument
    {
        ...
        $cbc = $xml->createElement('cbc:ReferenceID', $notaDeCredito->serieReferencia . '-' . $notaDeCredito->numeracionReferencia);
        $cbc = $DiscrepancyResponse->appendChild($cbc);
        $cbc = $xml->createElement('cbc:ResponseCode', $notaDeCredito->codigoAnulacion);
        $cbc = $DiscrepancyResponse->appendChild($cbc);
        ...
        $cbc = $xml->createElement('cbc:ID', $notaDeCredito->serieReferencia . '-' . $notaDeCredito->numeracionReferencia);
        $cbc = $IvReference->appendChild($cbc);
        $cbc = $xml->createElement('cbc:DocumentTypeCode', $notaDeCredito->codigoAlterno);
        $cbc = $IvReference->appendChild($cbc);
        ...
        $cbc->setAttribute('schemeID', $notaDeCredito->codigoCliente);
        $cbc = $PartyIdentification->appendChild($cbc);
        $PartyLegalEntity = $xml->createElement('cac:PartyLegalEntity');
        $PartyLegalEntity = $cac_party->appendChild($PartyLegalEntity);
        $cbc = $xml->createElement('cbc:RegistrationName');
        ...
        if ($impuesto > 0) {
            $cac_TaxSubtotal = $xml->createElement('cac:TaxSubtotal');
            $cac_TaxSubtotal = $cac_TaxTotal->appendChild($cac_TaxSubtotal);
            $cbc = $xml->createElement('cbc:TaxableAmount', number_format(round($subTotal, 2, PHP_ROUND_HALF_UP), 2, '.', ''));
            $cbc = $cac_TaxSubtotal->appendChild($cbc);
            $cbc->setAttribute('currencyID', $notaDeCredito->codiso);
            $cbc = $xml->createElement('cbc:TaxAmount', number_format(round($impuesto, 2, PHP_ROUND_HALF_UP), 2, '.', ''));
            $cbc = $cac_TaxSubtotal->appendChild($cbc);
