...
class XmlGenerator
{
    ...
    private static string $igvImpuestoGeneralALasVentas = "1000";

    /**
     * Declarar factura o boleta
    ...
    public static function createInvoiceXml(Venta $venta, array $detalles, Empresa $empresa, Sucursal $sucursal): DOMDocument
    {
        ...
        $impuesto = array_reduce($detalles, function ($acumulador, $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
        ...
        $total = array_reduce($detalles, function ($acumulador, $producto) {
        ...
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement('cbc:InvoiceTypeCode', $venta->codigoComprobante);
        $cbc = $Invoice->appendChild($cbc);
        $cbc->setAttribute('listID', "0101");
        $cbc = $xml->createElement('cbc:Note');
        $cbc->appendChild($xml->createCDATASection(NumberLleters::getResult(Util::roundingValue($total, 2), $venta->moneda)));
        $cbc = $Invoice->appendChild($cbc);
        $cbc->setAttribute('languageLocaleID', XmlGenerator::$igvImpuestoGeneralALasVentas);
        ...
        $cbc->setAttribute('schemeID', $venta->codigoTipoDocumento);
        $cbc = $PartyIdentification->appendChild($cbc);
        $PartyLegalEntity = $xml->createElement('cac:PartyLegalEntity');
        $PartyLegalEntity = $cac_party->appendChild($PartyLegalEntity);
        $cbc = $xml->createElement('cbc:RegistrationName');
        ...
        if ($venta->idFormaPago == FormaPago::CONTADO) {
        ...
        if ($venta->idFormaPago == FormaPago::CREDITO) {
            $PaymentTerms = $xml->createElement('cac:PaymentTerms');
            $PaymentTerms = $Invoice->appendChild($PaymentTerms);
            $cbc = $xml->createElement('cbc:ID', "FormaPago");
            $cbc = $PaymentTerms->appendChild($cbc);
            ...
            $cbc = $PaymentTerms->appendChild($cbc);
            $cbc = $xml->createElement('cbc:Amount', Util::roundingValue($total, 2));
        ...
        $cbc = $xml->createElement('cbc:TaxAmount', Util::roundingValue($impuesto, 2));
        $cbc->setAttribute('currencyID', $venta->codiso);
        ...
        if ($impuesto > 0) {
            ...
            $cbc = $xml->createElement('cbc:TaxableAmount', Util::roundingValue($sub_total, 2));
        ...
        $cbc = $xml->createElement('cbc:LineExtensionAmount', Util::roundingValue($sub_total, 2)); //
        $cbc = $cac_LegalMonetaryTotal->appendChild($cbc);
        $cbc->setAttribute('currencyID',  $venta->codiso);
        $cbc = $xml->createElement('cbc:TaxInclusiveAmount', Util::roundingValue($total, 2));
        $cbc = $cac_LegalMonetaryTotal->appendChild($cbc);
        $cbc->setAttribute('currencyID',  $venta->codiso);
        $cbc = $xml->createElement('cbc:PayableAmount', Util::roundingValue($total, 2)); //
        $cbc = $cac_LegalMonetaryTotal->appendChild($cbc);
        $cbc->setAttribute('currencyID',  $venta->codiso);
        ...
        foreach ($detalles as $detalle) {
            ...
            $cbc = $xml->createElement('cbc:InvoicedQuantity', Util::roundingValue($cantidad, 2));
            $cbc = $InvoiceLine->appendChild($cbc);
            $cbc->setAttribute('unitCode', $detalle->codigoMedida);
            $cbc = $xml->createElement('cbc:LineExtensionAmount', Util::roundingValue($importeBrutoTotal, 2));
            $cbc = $InvoiceLine->appendChild($cbc);
            $cbc->setAttribute('currencyID', $venta->codiso);
            ...
            $cbc = $xml->createElement('cbc:PriceAmount', Util::roundingValue($importeNeto, 2));
            $cbc = $cac->appendChild($cbc);
            $cbc->setAttribute('currencyID', $venta->codiso);
            ...
            $cbc = $xml->createElement('cbc:TaxAmount', Util::roundingValue($impuestoTotal, 2));
            $cbc = $taxtotal->appendChild($cbc);
            $cbc->setAttribute('currencyID', $venta->codiso);
            $taxtsubtotal = $xml->createElement('cac:TaxSubtotal');
            $taxtsubtotal = $taxtotal->appendChild($taxtsubtotal);
            $cbc = $xml->createElement('cbc:TaxableAmount', Util::roundingValue($importeBrutoTotal, 2));
            ...
            $cbc = $xml->createElement('cbc:Percent', Util::roundingValue($impuesto, 0));
            $cbc = $taxtcategory->appendChild($cbc);
            $cbc = $xml->createElement('cbc:TaxExemptionReasonCode', $detalle->codigo);
            $cbc = $taxtcategory->appendChild($cbc);


            if ($detalle->codigo == '10') {
                $igvcod = 'VAT';
                $igvnum = '1000';
                $igvname = 'IGV';
            } else {
            ...
            $cbc->appendChild($xml->createCDATASection($detalle->producto));
            $cbc = $item->appendChild($cbc);
            $price = $xml->createElement('cac:Price');
            $price = $InvoiceLine->appendChild($price);
            $cbc = $xml->createElement('cbc:PriceAmount', Util::roundingValue($precioBruto, 4));
     ...
     * @param Detalle[] $detalles
     */
    public static function createSummaryDocumentsXml(Venta $venta, array $detalles, Empresa $empresa, int $correlativo, DateTime $currentDate): DOMDocument
    {
        $sub_total = array_reduce($detalles, function ($acumulador, VentaDetalle $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $sub_total = floatval($producto->cantidad) * (floatval($producto->precio) / ($igv  + 1));
            return $acumulador + $sub_total;
        });
        ...
        $DocumentTypeCode = $xml->createElement('cbc:DocumentTypeCode', $venta->codigoComprobante);
        ...
        $AdditionalAccountID = $xml->createElement('cbc:AdditionalAccountID', $venta->codigoTipoDocumento);
        ...
        $TotalAmount = $xml->createElement('sac:TotalAmount', Util::roundingValue($total)); //
        ...
        if ($impuesto > 0) {
            ...
            $cbc = $xml->createElement('cbc:PaidAmount', Util::roundingValue($sub_total));
            $cbc->setAttribute('currencyID', $venta->codiso);
        ...
        // Monto IGV
        $cbc = $xml->createElement('cbc:TaxAmount', Util::roundingValue($impuesto));
        $cbc->setAttribute('currencyID', $venta->codiso);
        $cbc = $cac_TaxTotal->appendChild($cbc);
        ...
        if ($impuesto > 0) {
            ...
            $cbc = $xml->createElement('cbc:TaxableAmount', Util::roundingValue($sub_total));
            $cbc = $cac_TaxSubtotal->appendChild($cbc);
            ...
            $cbc = $xml->createElement('cbc:Percent', Util::roundingValue($detalles[0]->porcentaje));
            $cbc = $cac_TaxCategory->appendChild($cbc);
    ...
    /**
     * Anular notas de credito - boleta
     * @param Detalle[] $detalles
     */
    public static function createSummaryCreditNoteXml(NotaCredito $notaCredito, array $detalles, Empresa $empresa, int $correlativo, DateTime $currentDate): DOMDocument
    {
        $sub_total = array_reduce($detalles, function ($acumulador, NotaCreditoDetalle $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $sub_total = floatval($producto->cantidad) * (floatval($producto->precio) / ($igv  + 1));
            return $acumulador + $sub_total;
        });
        ...
        $date = new DateTime($notaCredito->fecha . "T" . $notaCredito->hora);
        ...
        $LineID = $xml->createElement('cbc:LineID', '1');
        $LineID = $SummaryDocumentsLine->appendChild($LineID);
        $DocumentTypeCode = $xml->createElement('cbc:DocumentTypeCode', $notaCredito->codigoComprobante);
        $DocumentTypeCode = $SummaryDocumentsLine->appendChild($DocumentTypeCode);
        $ID = $xml->createElement('cbc:ID', $notaCredito->serie . '-' . $notaCredito->numeracion);
        $ID = $SummaryDocumentsLine->appendChild($ID);

        $AccountingCustomerParty = $xml->createElement('cac:AccountingCustomerParty');
        $AccountingCustomerParty = $SummaryDocumentsLine->appendChild($AccountingCustomerParty);
        $CustomerAssignedAccountID = $xml->createElement('cbc:CustomerAssignedAccountID', $notaCredito->documento);
        $CustomerAssignedAccountID = $AccountingCustomerParty->appendChild($CustomerAssignedAccountID);
        $AdditionalAccountID = $xml->createElement('cbc:AdditionalAccountID', $notaCredito->codigoTipoDocumento);
        $AdditionalAccountID = $AccountingCustomerParty->appendChild($AdditionalAccountID);
        ...
        $TotalAmount = $xml->createElement('sac:TotalAmount', Util::roundingValue($total)); //
        $TotalAmount = $SummaryDocumentsLine->appendChild($TotalAmount);
        $TotalAmount->setAttribute('currencyID',  $notaCredito->codiso);
        ...
        if ($impuesto > 0) {
            $BillingPayment = $xml->createElement('sac:BillingPayment');
            $cbc = $xml->createElement('cbc:PaidAmount', Util::roundingValue($sub_total));
            $cbc->setAttribute('currencyID', $notaCredito->codiso);
            $cbc = $BillingPayment->appendChild($cbc);
     ...
     * Anular factura
     */
    public static function createVoidedDocumentsXml(Venta $venta, Empresa $empresa, int $correlativo, DateTime $currentDate): DOMDocument
    {
        $xml = new DomDocument('1.0', 'utf-8');
        // $xml->standalone         = true;
        $xml->preserveWhiteSpace = false;
     ...
     /**
     * Anular notas de credito - factura
     */
    public static function createVoidedCreditNoteXml(NotaCredito $venta, Empresa $empresa, int $correlativo, DateTime $currentDate): DOMDocument
    {
        $xml = new DomDocument('1.0', 'utf-8');
        // $xml->standalone         = true;
        $xml->preserveWhiteSpace = false;
    ...
    /**
     * Declarar guía de remisión
     * @param GuiaRemisionDetalle[] $detalles
     */
    public static function createDespatchAdviceXml(GuiaRemision $guiaRemision, array $detalles, Empresa $empresa): DOMDocument
    {
        ...
        foreach ($detalles as $detalle) {
            ...
            $cbc = $xml->createElement('cbc:DeliveredQuantity', $detalle->cantidad);
            $cbc->setAttribute('unitCode', $detalle->codigoMedida);
            $cbc = $cac_despatch_line->appendChild($cbc);
    ...
    /**
     * Declarar nota de crédito
     * @param NotaCreditoDetalle[] $detalles
     */
    public static function createCreditoNoteXml(NotaCredito $notaDeCredito, array $detalles, Empresa $empresa, Sucursal $sucursal): DOMDocument
    {
        $subTotal = array_reduce($detalles, function ($acumulador, $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $subTotal = floatval($producto->cantidad) * (floatval($producto->precio) / ($igv  + 1));
            return $acumulador + $subTotal;
        });

        $impuesto = array_reduce($detalles, function ($acumulador, $producto) {
            $igv = floatval($producto->porcentaje) / 100.00;
            $subTotal = floatval($producto->precio) / ($igv  + 1);
            $impuesto = floatval($producto->cantidad) * ($subTotal * $igv);
            return $acumulador + $impuesto;
        });
        ...
        $date = new DateTime($notaDeCredito->fecha . "T" . $notaDeCredito->hora);
        ...
        $cbc = $xml->createElement('cbc:CustomizationID', '2.0');
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement('cbc:ID', $notaDeCredito->serie . '-' . $notaDeCredito->numeracion);  // numero de factura
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement('cbc:IssueDate', $date->format('Y-m-d'));   // fecha de emision
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement('cbc:IssueTime', $date->format('H:i:s'));   // hora de emision
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement('cbc:Note');
        $cbc->appendChild($xml->createCDATASection(NumberLleters::getResult(round($total, 2, PHP_ROUND_HALF_UP),  $notaDeCredito->moneda)));
        $cbc = $Invoice->appendChild($cbc);
        $cbc->setAttribute('languageLocaleID', XmlGenerator::$igvImpuestoGeneralALasVentas);
        $cbc = $xml->createElement('cbc:DocumentCurrencyCode', $notaDeCredito->codiso);
        $cbc = $Invoice->appendChild($cbc);

        //MOTIVO ANULACION
        $DiscrepancyResponse = $xml->createElement('cac:DiscrepancyResponse');
        $DiscrepancyResponse = $Invoice->appendChild($DiscrepancyResponse);
        $cbc = $xml->createElement('cbc:ReferenceID', $notaDeCredito->serieVenta . '-' . $notaDeCredito->numeracionVenta);
        $cbc = $DiscrepancyResponse->appendChild($cbc);
        $cbc = $xml->createElement('cbc:ResponseCode', $notaDeCredito->codigoMotivo);
        $cbc = $DiscrepancyResponse->appendChild($cbc);
        $cbc = $xml->createElement('cbc:Description', $notaDeCredito->motivoAnulacion);
        $cbc = $DiscrepancyResponse->appendChild($cbc);
        ...
        $cbc = $xml->createElement('cbc:ID', $notaDeCredito->serieVenta . '-' . $notaDeCredito->numeracionVenta);
        $cbc = $IvReference->appendChild($cbc);
        $cbc = $xml->createElement('cbc:DocumentTypeCode', $notaDeCredito->codigoComprobanteVenta);
        $cbc = $IvReference->appendChild($cbc);
        ...
        $PartyIdentification = $xml->createElement('cac:PartyIdentification');
        $PartyIdentification = $cac_party->appendChild($PartyIdentification);
        $cbc = $xml->createElement('cbc:ID', $notaDeCredito->documento);
        $cbc->setAttribute('schemeID', $notaDeCredito->codigoTipoDocumento);
        $cbc = $PartyIdentification->appendChild($cbc);
        $PartyLegalEntity = $xml->createElement('cac:PartyLegalEntity');
        $PartyLegalEntity = $cac_party->appendChild($PartyLegalEntity);
        $cbc = $xml->createElement('cbc:RegistrationName');
        $cbc->appendChild($xml->createCDATASection($notaDeCredito->informacion));
        $cbc = $PartyLegalEntity->appendChild($cbc);
        ...
        $cac_TaxTotal = $Invoice->appendChild($cac_TaxTotal);
        $cbc = $xml->createElement('cbc:TaxAmount', Util::roundingValue($impuesto, 2));
        $cbc->setAttribute('currencyID', $notaDeCredito->codiso);
        $cbc = $cac_TaxTotal->appendChild($cbc);

        if ($impuesto > 0) {
            $cac_TaxSubtotal = $xml->createElement('cac:TaxSubtotal');
            $cac_TaxSubtotal = $cac_TaxTotal->appendChild($cac_TaxSubtotal);
            $cbc = $xml->createElement('cbc:TaxableAmount', Util::roundingValue($subTotal, 2));
            $cbc = $cac_TaxSubtotal->appendChild($cbc);
            $cbc->setAttribute('currencyID', $notaDeCredito->codiso);
            $cbc = $xml->createElement('cbc:TaxAmount', Util::roundingValue($impuesto, 2));
            $cbc = $cac_TaxSubtotal->appendChild($cbc);
        ...
        foreach ($detalles as $detalle) {
            ...
            $cbc = $xml->createElement('cbc:ID', $idlinea);
            $cbc = $InvoiceLine->appendChild($cbc);
            $cbc = $xml->createElement('cbc:CreditedQuantity', Util::roundingValue($cantidad, 2));
            $cbc = $InvoiceLine->appendChild($cbc);
            $cbc->setAttribute('unitCode', $detalle->codigoMedida);
            $cbc = $xml->createElement('cbc:LineExtensionAmount', Util::roundingValue($importeBrutoTotal, 2));
            $cbc = $InvoiceLine->appendChild($cbc);
